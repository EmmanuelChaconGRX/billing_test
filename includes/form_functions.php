...
if (!isset($HTML_FORM_FUNCTIONS) || $HTML_FORM_FUNCTIONS == "") {
...
#				$tmp = substr($tmp,0,3)."-".substr($tmp,3,3)."-".substr($tmp,6,4);
    ...
    function array2tbl($str, $params = "", $td_params = "", $tr1_params = "") {
        ...
        fputs($outstream, "<TABLE $params>\n");

        $rows = explode("-*-*", $str);
        foreach ($rows as $row) {

            fputs($outstream, "<TR $tr1_params>");
            ...
            foreach ($cols as $col) {
                ...
                fputs($outstream, "<TD $td_params $colp>$col</TD>");
            }
            fputs($outstream, "</TR>\n");
            $tr1_params = "";
        }
        fputs($outstream, "</TABLE>\n");
