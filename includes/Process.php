...
class Process implements \IteratorAggregate
{
    ...
    public function start(callable $callback = null/*, array $env = [*/)
    {
        ...
        if (2 <= \func_num_args()) {
            ...
            @trigger_error(sprintf('The %s::start() method expects a second "$env" argument since Symfony 3.3. It will be made mandatory in 4.0.','Process'), E_USER_DEPRECATED);
            $env = null;
        }
        ...
        while (null !== $this->callback || ($yieldOut && !feof($this->stdout)) || ($yieldErr && !feof($this->stderr))) {
            ...
            if ($yieldErr) {
                ...
                if (isset($err[0])) {
                    ...
                    return array(Process::ERR => $err);
                }
            }

            if (!$blocking && !isset($out[0]) && !isset($err[0])) {
                return array(Process::OUT => '');
            }
    ...
    public function setInput($input)
    {
        ...
        $this->input = $input; // ProcessUtils::validateInput(__METHOD__, $input);
    ...
    protected function buildCallback(callable $callback = null)
    {
        ...
        $out = self::OUT;
        $that=$this;
        return function ($type, $data) use ($callback, $out, $that) {
            if ($out == $type) {
                $that->addOutput($data);
            } else {
