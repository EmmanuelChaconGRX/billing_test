...
class Template {
    ...
    function set_var($varname, $value = "", $append = false) {
        // added by jyancey on 6-28-2010 if the user does not have the access
        // to view totals then the values of those fields are replaced by "Unavailable"
        global $viewtotals;
        if (!isset($viewtotals)) {
            global $permissions;
            global $message;
            if (!$permissions) {
                global $userdata; // userdata should always be here
                $permissions = json_decode($userdata["permissions"], true);
            }
            $viewtotals = $permissions[$message['tp']]['VIEWTOTALS'];
        ...
        if (!$viewtotals) {
            $totalfields[] = 'PO104';
            $totalfields[] = 'PO1LINETOTAL';
            $totalfields[] = 'POCLINETOTAL';
            $totalfields[] = 'POC06';
            $totalfields[] = 'TOTAL';
        }
        if (!is_array($varname)) {
            if (!empty($varname)) {
                ...
                if (in_array($varname, $totalfields)) {
                    if ($this->get_var('EDITFLAG') == "NONE") { // this check was added to make sure tha we never send out bad edi
                        $value = "Unavailable";
                    }
                }
    ...
    function subst($varname) {
        // rewritten by jyancey on 6-29-2010
        // this function was VERY inefficient and I rewrote it to be better.
        ...
        // we are doing the slashes replace to make sure that we don't have a problem with this later on.
        // It does shave off about 30 milliseconds but it isn't worth the risk.

        $str = @preg_replace(array('/\\\\/', '/\$/'), array('\\\\\\\\', '\\\\$'), @preg_replace($this->varkeys, $this->varvals, $this->get_var($varname)));
        // $str = @preg_replace($this->varkeys, $this->varvals, $this->get_var($varname));
        if ($this->debug & 4) {
            echo "<p><b>subst:</b> completed: " . strlen($str) . " bytes in length</p>\n";
        }
