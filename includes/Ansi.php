...
function consolelog($msg,$level=LOG_DEBUG) {
    ...
    if(LOG_LEVEL >= $level) {
        echo Ansi::tagsToColors("$color$msg".PHP_EOL);
    }
}
function consoleerr($msg) { consolelog($msg,LOG_ERR); }
function consolewarn($msg) { consolelog($msg,LOG_WARNING); }
function consolenoti($msg) { consolelog($msg,LOG_NOTICE); }
function consoleinfo($msg) { consolelog($msg,LOG_INFO); }
function consoledebug($msg) { consolelog($msg,LOG_DEBUG); }
...
function tick_handler()
{
    ...
    consoledebug($trace[1]['file'].'#'.$trace[1]['line']);
}
