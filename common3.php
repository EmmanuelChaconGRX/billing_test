<?php
// This a candidate for a Symfony-Service
// Copied from common2.php, modified to output arrays(tables) instead of HTML
include(__DIR__ . '/../env.php');
include(__DIR__ . '/edi_traffic_queries_new.php');
#print_r ($_SERVER);die();
...
function billing_array($plan_id, $doc_qty,$dropship_qty=0) {
    ...
    if($plan_["dropship"]=="Y") {
            $lines[] = array("description" => "Dropship traffic","qty" =>n($dropship_qty),"cost" => n($plan_["dropship_base_price"]),"subtotal" => n($plan_["dropship_base_price"]*$dropship_qty));
    }else{
...
function calculate_billing($plan_id, $doc_qty, $print = true,$dropship_qty=0) {
    ...
    if ($print) {
        ...
        print array2tbl(join($tbl_rows, "\n-*-*"), "STYLE='FONT-FAMILY:verdana;FONT-SIZE:10px' CELLSPACING=0 CELLPADDING=2 BORDER=1", "NOWRAP", "STYLE='font-weight:bold'");
    }
...
function n($n, $d = 2) {
    return number_format(floatval($n), $d,'.',''); // Comma for Thousands was trowing off calculations for DS
}
...
function report_tp($tp, $no3KBlocks = false, $unit_size = 3000, $partnerFilter = null,$dropship=false) {
    global $timer;
    global $f;
...
//error_log('valor allQty---' . print_r($allQty, true));
    global $userid;
    $queryBuilder = new EcsQueries($tp,$year,$month);
    $queryBuilder->setDropship($dropship)
    ...
    //$q1 =$queryBuilder->buildQuery('QUERY_REC_EDI');
	//var_dump($QUERY_REC_EDI);
    $out[$ms_db_name]['RECV'] = report_query($QUERY_REC_EDI, false, $unit_size, $allQty, 'RECV');
...
//		$timer->report("FINISHED report_query(QUERY_REC_EDI)");
    ...
    $out[$ms_db_name]['SENT'] = report_query($QUERY_SENT_EDI, false, $unit_size, $allQty, 'SENT');
//		$timer->report("FINISHED report_query(QUERY_SENT_EDI)");
    ...
    $out[$ms_db_name]['SENTQ'] = report_query($QUERY_BY_OUTPUTQUEUE, false, $unit_size, $allQty, 'SENTQ');
//		$timer->report("FINISHED report_query(QUERY_BY_OUTPUTQUEUE)");
    ...
    $out[$ms_db_name]['RECVQ'] = report_query($QUERY_BY_CHANNEL, false, $unit_size, $allQty, 'RECVQ');
    #$out .= "$total_quantity ---<BR>";
    ...
    if ($tp == 388 ) {
        ...
        $out[$ms_db_name]['SENTFTP'] = report_query($QUERY_SENT_FTP, false, $unit_size, $allQty, 'SENTFTP');
        #$edi_total_quantity = $total_quantity;
    }
    ...
    if ($tp == 295) {
        ...
        //$out .= "<BR><BR>$f[20]EDI Data from/to FTP:$f[0]<BR>";
        $out[$ms_db_name]['RECVFTP'] = report_query($QUERY_SENT_FTP, false, $unit_size, $allQty, 'RECVFTP');
        #$edi_total_quantity = $total_quantity;
    }
    ...
    if ($tp == 121) {
        ...
        $out[$ms_db_name]['RECVPOP3'] = report_query($QUERY_REC_NON_EDI_121, false, $unit_size, $allQty, 'RECVPOP3');
        //$out .= "<BR><BR>$f[20]FTP Outbound Data to MIDSHIP (Non EDI):$f[0]<BR>";
        $QUERY_SENT_NON_EDI_121 = $sent_121_;
        $out[$ms_db_name]['SENTPOP3'] = report_query($QUERY_SENT_NON_EDI_121, false, $unit_size, $allQty, 'SENTPOP3');
        #$edi_total_quantity = $total_quantity;
    }
    if ($tp == 353) {
        //$out .= "<BR><BR>$f[20]FTP Outbound Data TO AKRON (Duplicate):$f[0]<BR>";
        $out[$ms_db_name]['SENTFDUP'] = report_query($QUERY_SENT_EDI_AKRON, false, $unit_size, $allQty, 'SENTFDUP');
        #$out .= "$total_quantity ---<BR>";
    }
    if ($tp == 178) {
        //$out .= "<BR><BR>$f[20]EDI Data from HomeDepot and Academy (Duplicate):$f[0]<BR>";
        $out[$ms_db_name]['RECVDUP'] = report_query($QUERY_REC_EDI_FANMATS_HD, false, $unit_size, $allQty, 'RECVDUP');
        #$out .= "$total_quantity ---<BR>";
    }
    if ($tp == 455) {
        //$out .= "<BR><BR>$f[20]Inbound EDI Data from Office Depot (Duplicate):$f[0]<BR>";
        $out[$ms_db_name]['RECVDUP'] = report_query($QUERY_REC_EDI_INNOVATIVESD, false, $unit_size, $allQty, 'RECVDUP');
        #$out .= "$total_quantity ---<BR>";
    }
    if ($tp == 770) {
        //$out .= "<BR><BR>$f[20]Inbound EDI Data from PACCAR (Carbon Copy):$f[0]<BR>";
        $out[$ms_db_name]['RECVCC'] = report_query($QUERY_REC_EDI_MMFASTENERS_CC, false, $unit_size, $allQty, 'RECVCC');
        //$out .= "$total_quantity ---<BR>";
        #$edi_total_quantity = $total_quantity;
    }
    if ($tp == 840) {
        //$out .= "<BR><BR>$f[20]Inbound 850s and 852s (Carbon Copy):$f[0]<BR>";
        $out[$ms_db_name]['RECVCC'] = report_query($QUERY_REC_EDI_NOVA_CC, false, $unit_size, $allQty, 'RECVCC');
        //$out .= "$total_quantity ---<BR>";
    ...
    if ($tp == 552 || $tp == 806) { #Thill/Grilldaddy/FitnessIQ/
        ...
        $out[$ms_db_name]['RECVCC'] = report_query($QUERY_REC_EDI_THILL_850s_CC, false, $unit_size, $allQty, 'RECVCC');
        //$out .= "$total_quantity ---<BR>";
        //$out .= "edi_total_quantity: $edi_total_quantity ---<BR>";
        #$edi_total_quantity = $total_quantity;
    }
...
function isDropshipWebEDI($message) {
    if($message['doctype'] == '850') {
        if(in_array($message['tp'],array(2148,2700,18,3341,743,2656,2513,816,1448,3759,3488,3548,2241,751,3070,3537,742,2931))) {
            return true;
        }
        $BEG02 = mysql_value("SELECT e.data FROM webedi30.Elements e WHERE e.`index`=2 AND e.nodepointer='{$message['rootnode']}'");
//	if($message['PK_id']==22635535) { var_dump($BEG02); var_dump(trim($BEG02) =='DS'); die(); }
        if (trim($BEG02) =='DS') {
	    return true;
	}
	if(in_array($message['tp'],array(743,3548,3488,2656,2513,2812,816,1448,3759,2241,751,3070,3537,742,2931,1715,255,1179,816,2766,272))) { // Home Depot CHUB
            $result = mysql_rows("SELECT e.data FROM webedi30.Nodes n JOIN webedi30.Elements e ON n.PK_id = e.nodepointer WHERE n.title='REF' AND e.`index`=1 AND e.rootnode='{$message['rootnode']}'");
            foreach($result as $i=>$row) {
                if($row['data']=='CO') return true;
            }
        }
	if(in_array($message['tp'],array(152))) {
	    $result = mysql_rows("SELECT e.data FROM webedi30.Nodes n JOIN webedi30.Elements e ON n.PK_id = e.nodepointer WHERE n.title='MSG' AND e.`index`=1 AND e.rootnode='{$message['rootnode']}'");
	    foreach($result as $i=>$row) {
                if($row['data']=='DROP SHIP') return true;
            }
	}
    }else{
        $po_message=mysql_row("SELECT m.* FROM webedi30.Messages m JOIN webedi30.Associations a ON m.PK_id=a.associate AND m.doctype=850 WHERE a.msgid='{$message['PK_id']}'");
        if($po_message) {
            return isDropshipWebEDI($po_message);
        }else{ 
//	    echo "Po not found\n";
	} 
    }
...
function isDropship($tpid,$batchId,$batchTitle) {
    if(preg_match('/WEBEDI-MSGID#([0-9]+)/', $batchTitle,$match)) {
        $msgid = $match[1];
    }else{
        $msgid = mysql_value("SELECT b.msgid FROM webedi30.MessageBatches b WHERE b.Batchid='$batchId'");
    }
    if($msgid) {
        $message = mysql_row("SELECT * FROM webedi30.Messages m WHERE m.PK_id='$msgid'");
        return isDropshipWebEDI($message);
    }
    return false;
}

function report_query($query, $no3KBlocks = false, $unit_size = 3000, $allQty, $traffic_class) {
    global $timer;
    ...
    if (mssql_num_rows($result) > 0) {
        $keys = array("BatchId", "Title", "Size", "Date", "TP_ID", "TP_Name", "Type", "Control", "Units");
        while ($row = mssql_fetch_array($result)) {
            ...
            if ($filterDups && foundDuplicate($userid, $row)) {
                $row['Duplicate'] = true;
            } else {
                $row['Duplicate'] = false;
            }
            $row['TrafficType'] = $traffic_class;
            
            if(strpos($row['UserCreated'], 'DROPSHIP') !== FALSE) {
                $row['Dropship'] = true;
            }else{
                $row['Dropship']=isDropship($userid,$row['BatchId'],$row['BatchTitle']);
            }
            
            $row['CarbonCopy'] = (strpos($traffic_class, 'CC') !== FALSE);
...
//						$tmp_row=mssql_fetch_array($qty_result);
                ...
                $units = ceil(floatval($size) / floatval($unit_size));

                // die("Size: $size, Units; $units, Qty: {$row["Qty"]} no3KBlocks: $no3KBlocks, unit_size:$unit_size\n");
                ...
                $row['Units'] = $units;
 //error_log('impresion del row= ' . print_r($row, true));
                insertDetail($userid, $row);
                if (false == $row['Duplicate']) {
                    $values = array("{$row["BatchId"]}", "{$row["BatchTitle"]}", "{$row["ContentSize"]}", "{$row["Created"]}", "{$row["tpid"]}[$tpid]", "{$row["tpname"]}", "{$row["type"]}", "{$row["control"]}", "$units");
...
function createDabatase($custid) {
    global $pdo_db;
    global $dups_table;
    if(!isset($dups_table)) {
        $dups_table=array();
    ...
    $sql="SELECT t1.foreign_reference,t2.prop_value FROM billing.billing_workitems t1 
            JOIN billing.billing_workitem_props t2 ON t1.id = t2.workitem_id 
            WHERE t1.customer_reference=:custid AND  t2.prop_name='hash';";
    $stmt=$pdo_db->prepare($sql);
    $stmt->execute(array('custid'=>$custid));
    while($row=$stmt->fetch(PDO::FETCH_ASSOC)) {
        if(!isset($dups_table[$row['prop_value']])) {
            $dups_table[$row['prop_value']]=array();
        }
        $dups_table[$row['prop_value']][]=$row['foreign_reference'];
    }
}
...
//if($row['BatchId']!=120883791){
    if (!isset($row['control']) || !isset($row['type']) || $row['type']=='FLATFILE' || $row['control']=='N/A') {
        return false;
    }
//}
    error_log('Checking Dup for test' . print_r($row, true));
    if ($pdo_db == null) {
        createDabatase($custid);
    }
    if (isset($dups_table[$row['hash']])) {
        $data = $dups_table[$row['hash']]; // get first
        //error_log('found' . print_r($data, true));
        if ("{$data[0]}" === "{$row['BatchId']}") {
            error_log('Dup is ignored - first');
            return false;
        }//else
        error_log("Dup_is_found_First#:'{$data[0]}' !== Batch#:'{$row['BatchId']}'");
        return true;
    }
    return false;
}

function insertDetail($custid, $row,$bill_code=0) {
error_log('row messages_test= ' . print_r($row, true));
    global $pdo_db;
    global $ms_db_name;
    global $dups_table;
    ...
    $stmt3= $pdo_db->query("SELECT id FROM billing.billing_workitems WHERE foreign_reference='{$row['BatchId']}'");
    $id=$stmt3->fetchColumn(); 
    $stmt3->closeCursor();
    $params3 = array(
        'batchid' => $row['BatchId'],
        'server' => $ms_db_name,
    ...
    if(false === $id) {
        $query3 = "INSERT INTO billing.billing_workitems (foreign_reference,server,time_stamp,customer_reference,billing_code_id,title,qty1,qty2,qty3) "
            . " VALUES (:batchid,:server,:time,:customer,:bill_code,:title,:units,:size,:qty);";
    }else{
        $query3 = "UPDATE billing.billing_workitems SET time_stamp=:time, customer_reference=:customer, billing_code_id=:bill_code, title=:title, qty1=:units, qty2=:size, qty3=:qty WHERE id='$id';";
	unset($params3['batchid']);
        unset($params3['server']);
    }
    $query4 = "REPLACE INTO billing.billing_workitem_props (workitem_id,prop_name,prop_value) "
    ...
    if ($ret2 !== false) {
        ...
        if($is_duplicate==1){
            $stmt_i= $pdo_db->query("SELECT prop_value FROM billing.billing_workitem_props WHERE workitem_id='$id' AND prop_name='Duplicate' ");
            $prop_value=$stmt_i->fetchColumn();

            $count_tp_mt = mysql_value("SELECT count(*) FROM webedi.webedi_tp_monthly_traffic WHERE month=$month AND year=$year AND FK_tp=$custid");
            if($prop_value==false && $count_tp_mt==1 ){ //we subtract duplicates
$fechaRegistro = strtotime(date("Y-m-d", strtotime($row['Created'])));
$fechaHoy = strtotime(date("Y-m-d",strtotime(date('Y-m-d')."- 2 days")));
if($fechaHoy>= $fechaRegistro){
                mysql_query("UPDATE webedi.webedi_tp_monthly_traffic SET traffic = traffic-'$qty1_rest' WHERE FK_tp=$custid AND year=$year AND month=$month");
                error_log('new_qty1_rest= ' . print_r($qty1_rest, true));
}
            }else{
                error_log('you_already_have qty1_rest= ' . print_r($qty1_rest, true));
            }
        }
        foreach ($row as $k => $v) {
            if (!is_numeric($k)) {
                $ret3=$stmt2->execute(array('id' => $id, 'name' => $k, 'value' => $v));
        		if($ret3 === false) {
        		    error_log($query4 . print_r(array('id' => $id, 'name' => $k, 'value' => $v),true));
        		    die(var_dump($stmt2->errorInfo()));
        		}
            }
        }
        // Add into Dup filter
        if(!isset($dups_table[$row['hash']])) {
            $dups_table[$row['hash']]=array();
        }
        $dups_table[$row['hash']][]=$row['BatchId'];
    } else {
