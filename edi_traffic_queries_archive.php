...
if ($tpid == 121) {
    ...
    $rec_121_ = "SELECT * FROM Archive_AS2A.dbo.ArchiveBatches AS Batches WHERE BatchTitle LIKE 'Packing List From MID-SHIP - Five Rivers Distribution, Van Buren, AR%' AND $DATE_W";
    #$sent_121_ = "SELECT * FROM Deliveries WHERE OutputQueue = '1073743018' AND $DATE_W";
    $sent_121_ = "SELECT created,recipients as BatchTitle, size as ContentSize, 'Truck File' as type, 'MIDSHIP' as tpname FROM Archive_AS2A.dbo.ArchiveDeliveries AS Deliveries WHERE OutputQueue='1073743018' AND $DATE_W";
    $edi_dir = "dts1312";
...
$QUERY_BY_CHANNEL = "
		SELECT
			...
			Created,
                        Batches.UserCreated
		FROM
			Archive_AS2A.dbo.ArchiveBatches AS Batches WITH (NOLOCK) INNER JOIN DTSBilling.dbo.DTS_ECS_Channels c ON DataChannel=c.item_id
		WHERE
			c.tpid = $tpid
			AND c.EDI = 0
...
$QUERY_BY_CHANNEL_EDI = "
		...
		FROM
		    Archive_AS2A.dbo.ArchiveBatches AS Batches WITH (NOLOCK)
		    INNER JOIN DTSBilling.dbo.DTS_ECS_Channels c ON DataChannel=c.item_id
		    RIGHT OUTER JOIN Archive_AS2A.dbo.ArchiveDTSEnvelopes AS DTSEnvelopes WITH (NOLOCK)
		    RIGHT JOIN Partners AS FromPartner
		    RIGHT JOIN Partners AS ToPartner
		    RIGHT JOIN Archive_AS2A.dbo.ArchiveDTSDocuments AS DTSDocuments WITH (NOLOCK) ON ToPartner.tpid = DTSDocuments.tpid_recipient
...
$QUERY_BY_OUTPUTQUEUE = "
		...
		FROM 
			Archive_AS2A.dbo.ArchiveDeliveries AS Deliveries WITH (NOLOCK)
			INNER JOIN DTSBilling.dbo.DTS_ECS_Channels c ON OutputQueue=c.item_id
			INNER JOIN Archive_AS2A.dbo.ArchiveBatches AS Batches WITH (NOLOCK) ON Batches.BatchID = Deliveries.BatchID
		WHERE 
			1=1
			AND EDI = 0
			AND tpid = $tpid
