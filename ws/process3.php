<?php
// This version adds the capability to run the process in a Idempotent manner on any Month/Year
set_time_limit(0);
...
try {
    ...
    foreach ($list as $i => $entry) {
        ...
        foreach (array('AS2A', 'AS2B', 'AS2C') as $db) {
            ...
            $cmd = "\$(which php) -f $workingPath/traffic_api_1.php -- --start_date='$start_date' --end_date='$end_date' --form_userid={$entry['PK_id']} --db=$db 2>>/var/log/php-billing-error.log ";
...
$email->setSubject("The traffic calculation from $start_date to $end_date has run successfully.");
$email->setMessage("The traffic calculation from $start_date to $end_date has run successfully.");
