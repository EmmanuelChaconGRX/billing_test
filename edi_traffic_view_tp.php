   ...
   $filterDocsInclusive = getInclusiveDocFilter($userid);
    ...
    if ((isset($_REQUEST["form_submit"]) && $_REQUEST["form_submit"] == "Submit" ) || $updating || $userid <> '') {
        ...
        if (true) { // $tpid!='')
            ...
            $dtlTemplate->set_var(array("SCRIPT"=>"tptrafficsubview.php", "NAME" => "WinProd", "DB" => "AS2A", "QSTRING" => $_SERVER['QUERY_STRING']));
            ...
            $dtlTemplate->set_var(array("SCRIPT"=>"tptrafficsubview.php", "NAME" => "AS2B", "DB" => "AS2B", "QSTRING" => $_SERVER['QUERY_STRING']));
            ...
            $dtlTemplate->set_var(array("SCRIPT"=>"tptrafficsubview.php", "NAME" => "AS2C", "DB" => "AS2C", "QSTRING" => $_SERVER['QUERY_STRING']));
            ...
            $dtlTemplate->set_var(array("SCRIPT"=>"tptrafficsubview.php", "NAME" => "WinProd 2", "DB" => "AS2D", "QSTRING" => $_SERVER['QUERY_STRING']));
            ...
            if ($def_schema) {
                ...
                mssql_select_db($ms_db_name2, $ms_db2) or die("Could not select DB: $ms_db_name2");

                $mrs = mssql_query("SELECT * FROM partners Where local_id1 = '{$partner["webtpid"]}'") or die(mssql_message());
                $row = mssql_fetch_array($mrs);
                $tpid = $row["tpid"];   #Customer's ID in ECS database
                if ($tpid && 'y' != $_GET['alldocs']) {
                    $filterDocsInclusive = getInclusiveDocFilter($userid);
                    include "edi_traffic_queries.php";
                    report_tp($tpid, $no3KBlocks);
                }
                ...
                mssql_select_db($ms_db_name3, $ms_db3) or die("Could not select DB: $ms_db_name3");
                $mrs = mssql_query("SELECT * FROM partners Where local_id1 = '{$partner["webtpid"]}'") or die(mssql_message());
                $row = mssql_fetch_array($mrs);
                $tpid = $row["tpid"];   #Customer's ID in ECS database
                if ($tpid && 'y' != $_GET['alldocs']) {
                ...
                # AS2D
                /*$ms_db_name4 = "AS2D";
                $ms_db4 = mssql_connect($ms_db_host, $ms_db_user, $ms_db_pass) or die("Unable to connect!");
                mssql_select_db($ms_db_name4, $ms_db4) or die("Could not select DB: $ms_db_name4");
                $mrs = mssql_query("SELECT * FROM partners Where local_id1 = '{$partner["webtpid"]}'") or die(mssql_message());
                $row = mssql_fetch_array($mrs);
                $tpid = $row["tpid"];   #Customer's ID in ECS database
                if ($tpid && 'y' != $_GET['alldocs']) {
