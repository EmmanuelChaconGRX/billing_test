...
$recentCancel = '';
if ($showTrend) {
    $recentCancel = 'OR cancel_date > (CURRENT_DATE() - INTERVAL 3 MONTH)';
}
...
WHERE
    ((type = 'TP' AND pa.value IS NOT NULL)
        ...
        AND ((status = 'Active' OR status = 'InActive') $recentCancel)
ORDER BY $OBY $ODIR;");
...
<HTML>
    ...
    <BODY XXONFOCUS='body_focus();'>
        ...
        <TEXTAREA STYLE='visibility:hidden;display:none' ID='tarea' COLS=50 ROWS=5></TEXTAREA>
            <?php
            print "<font style='font-family:verdana;font-size:20;font-weight:bold'>MAIN (Billing Cycle: $year-$month)</font><BR>";
            ...
            foreach ($partners as $partner) {
                ...
                if ($billing) {
                   $billingC=0;//DS-4813
                    if($billing["billing_schema"]) {
                        $newdate= $billing["date"];
                        $start_date =date("Y-m-d",strtotime($newdate."- 1 days"));
                        $end_date = date('Ym01', strtotime("$year-$month-1 +1 month"));
                        $billingC = mysql_value("SELECT SUM(qty1)
                        FROM (SELECT wi.*,
                                ...
                                WHERE  wi.customer_reference = '$userid' AND wi.time_stamp BETWEEN '$start_date' AND '$end_date'
                                GROUP BY wi.id
                                HAVING `dup` = 0 ) t;");
                    }
                    $billing["traffic"]=$billing["traffic"] + $billingC;
                ...
                $links = "<A HREF='edi_traffic_view_tp_1.php?form_month=$month&form_year=$year&form_userid=$userid' ONCLICK='flag($userid);' TARGET='_new'>[EDIT]</A>";
                $links2 = '';
